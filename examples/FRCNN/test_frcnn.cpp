...
int main(int argc, char** argv){
  ...
  FRCNN_API::Frcnn_wrapper wrapper(proto_file, model_file, default_config_file);
  std::vector<boost::shared_ptr<caffe::Blob<float> >> input;
  std::vector<boost::shared_ptr<caffe::Blob<float> >> output;
  cv::Mat out_image;
  FRCNN_API::Frcnn_wrapper::prepare(cv_image, out_image);
  FRCNN_API::Frcnn_wrapper::preprocess(out_image, input);
  wrapper.predict(input, output);
  FRCNN_API::Frcnn_wrapper::postprocess(out_image, output, results);
  ...
  for(int ir = 0; ir < results.size(); ir++){
      ...
      && results[ir].id == 7){
          ...
          cv::rectangle(out_image, rect, cv::Scalar(255, 0, 0), 2);
